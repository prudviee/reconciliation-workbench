...
- [x] **OPS-T07 — Implement idempotent workspace cleanup** (`OPS-009`, `OPS-015`, `OPS-016`; `OPS-A05`, `OPS-A07`, `OPS-A15`)
  - Change: `WORKSPACE_CLEANUP` work items claim `WorkspaceCleanupRequest` rows; `workspaces.cleanup.purge_workspace` re-locks the workspace, re-verifies `REVOKED`/`DELETED` state and the claiming attempt's fencing token, then deletes the `Workspace` row itself — every workspace-owned table across every app already cascades from it — and deletes each snapshotted artifact's bytes only after that transaction commits; the absence of the `Workspace` row is the durable completion signal (redelivery is a no-op by construction, since there is nothing left to purge). Deployment documentation states both the seven-day live `Workspace.expires_at` window this job enforces and the storage/database providers' separate longer backup-retention window — `OPS-T09`'s deliverable.
...
- [x] **OPS-T08 — Extend health reporting and structured observability** (`OPS-003`, `OPS-011`, `OPS-014`; `OPS-A08`, `OPS-A13`, `OPS-A14`)
  - Change: `foundation.management.commands.worker`'s heartbeat-only loop is replaced with a real claim loop that calls `jobs.services.claim_and_execute` for every registered `JobKind` each cycle, keeping the existing heartbeat-file write for the local Compose healthcheck and adding a new `jobs.WorkerHeartbeat` database row updated the same cycle; `/health/ready` reports web readiness, database reachability, and worker liveness/freshness (computed from the most recent `WorkerHeartbeat` row) as independent fields; `observability.logging.EVENT_FIELDS` gains book/scope/run/import/job identifiers and stage duration; `jobs.services.claim_and_execute` emits one structured event per claimed item reusing `hash_workspace_ref` (extracted from `observability.middleware.workspace_reference` into `observability.logging` for reuse outside the request cycle).
...
- [x] **OPS-T09 — Ship deployment configuration and measured capacity** (`OPS-010`, `OPS-012`, `OPS-013`, `OPS-016`; `OPS-A06`, `OPS-A13`, `OPS-A15`)
  - Change: `docs/07-deployment.md` documents the resolved target (managed PostgreSQL, S3-compatible object storage, HTTPS ingress, environment-held secrets, migrations executed as a controlled release step, versioned web/worker images) and the retention disclosure text `OPS-T07`'s behavior requires; two new tests prove `configured_artifact_store()`'s settings-driven backend selection; one real capacity measurement run against this repository's own PostgreSQL, recorded honestly including a missed target.
