...
The browser now covers workspace creation, source preparation, durable reconciliation jobs, selected-run history, side-by-side case evidence, reasoned manual linking with rerun persistence, complete server-side case discovery, safe CSV/JSON exports, global-allocation explanations, and truthful run progress/failure states. The complete submission journey requires no JavaScript and keeps historical run facts separate from the current review queue. Uploaded files stay in the private persistent Docker volume; no cloud storage account is required.
